...
func findgoversion() string {
	...
	if isfile(path) {
		...
		if b != "" {
			...
			// but it is easier to change cmd_local/dist than to try to make changes to
...
var unreleased = []string{
	"src/cmd_local/newlink",
	"src/cmd_local/objwriter",
	"src/debug/goobj",
	"src/old",
}
...
func runInstall(pkg string, ch chan struct{}) {
	...
	ispkg := !strings.HasPrefix(pkg, "cmd_local/") || strings.Contains(pkg, "/internal/") || strings.Contains(pkg, "/vendor/")
	...
	if IsRuntimePackagePath(pkg) {
		...
		if os.Getenv("GOEXPERIMENT") == "regabi" {
			...
			// cmd_local/go/internal/work/gc.go.
			//
	...
	pkgName := pkg
	if strings.HasPrefix(pkg, "cmd_local/") && strings.Count(pkg, "/") == 1 {
		pkgName = "main"
	}
...
func shouldbuild(file, pkg string) bool {
	...
	for _, p := range strings.Split(readfile(file), "\n") {
		...
		if code == "package main" && pkg != "cmd_local/go" && pkg != "cmd_local/cgo" {
			return false
		}
...
var toolchain = []string{"cmd_local/asm", "cmd_local/cgo", "cmd_local/compile", "cmd_local/link"}

// The bootstrap command runs a build from scratch,
// stopping at having installed the go_bootstrap command.
//
// WARNING: This command runs after cmd_local/dist is built with Go 1.4.
// It rebuilds and installs cmd_local/dist with the new toolchain, so other
...
func cmdbootstrap() {
	...
	if debug {
		// cmd_local/buildid is used in debug mode.
		toolchain = append(toolchain, "cmd_local/buildid")
	}
	...
	xprintf("Building Go bootstrap cmd_local/go (go_bootstrap) using Go toolchain1.\n")
	install("runtime") // dependency not visible in sources; also sets up textflag.h
	install("cmd_local/go")
	if vflag > 0 {
		xprintf("\n")
	}
	...
	// (cmd_local/asm, cmd_local/cgo, cmd_local/compile, cmd_local/link)
	// using Go 1.4's toolchain and go command.
	// Then we built the new go command (as go_bootstrap)
	// using the new toolchain and our own build logic (above).
	//
	//	toolchain1 = mk(new toolchain, go1.4 toolchain, go1.4 cmd_local/go)
	//	go_bootstrap = mk(new cmd_local/go, toolchain1, cmd_local/dist)
	//
	// The toolchain1 we built earlier is built from the new sources,
	// but because it was built using cmd_local/go it has no build IDs.
...
func checkNotStale(goBinary string, targets ...string) {
	...
	if strings.Contains(out, "\tSTALE ") {
		...
		for _, target := range []string{"runtime/internal/sys", "cmd_local/dist", "cmd_local/link"} {
...
// Cannot use go/build directly because cmd_local/dist for a new release
...
// name in cmd_local/internal/objabi/path.go.
