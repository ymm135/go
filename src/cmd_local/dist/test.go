...
func (t *tester) run() {
	...
	// Now that for example "go install cmd_local/compile" does not
	// also install runtime (you need "go install -i cmd_local/compile"
	...
	// where cmd_local/dist is invoked many times. This just slows that
	// down (Issue 24300).
...
// detail between the Go build system and cmd_local/dist for
...
func (t *tester) registerTests() {
	...
	// This test needs its stdout/stderr to be terminals, so we don't run it from cmd_local/go's tests.
	// See issue 18153.
	if goos == "linux" {
		t.tests = append(t.tests, distTest{
			name:    "cmd_go_test_terminal",
			heading: "cmd_local/go terminal test",
			fn: func(dt *distTest) error {
				t.runPending(dt)
				timelog("start", dt.name)
				defer timelog("end", dt.name)
				...
				cmd.Dir = filepath.Join(os.Getenv("GOROOT"), "src/cmd_local/go/testdata/testterminal18153")
				cmd.Stdout = os.Stdout
				cmd.Stderr = os.Stderr
				return cmd.Run()
			},
	...
	if goos != "android" && !t.iOS() && goos != "js" && goos != "plan9" {
		t.tests = append(t.tests, distTest{
			...
			fn: func(dt *distTest) error {
				if t.compileOnly {
					t.addCmd(dt, "src", "go", "build", "-o", os.DevNull, filepath.Join(goroot, "src/cmd_local/api/run.go"))
					return nil
				}
				t.addCmd(dt, "src", "go", "run", filepath.Join(goroot, "src/cmd_local/api/run.go"))
				return nil
			},
		})
	}
...
func (t *tester) internalLink() bool {
	...
	if gohostarch == "ppc64le" {
		// linkmode=internal fails on ppc64le because cmd_local/link doesn't
...
func (t *tester) raceTest(dt *distTest) error {
	...
	// t.addCmd(dt, "src", t.goTest(),  "-race", "-run=TestParallelTest", "cmd_local/go")
	if t.cgoEnabled {
...
// cmd_local/internal/sys.RaceDetectorSupported, which can't be used here
// because cmd_local/dist has to be buildable by Go 1.4.
...
// mSanSupported is a copy of the function cmd_local/internal/sys.MSanSupported,
// which can't be used here because cmd_local/dist has to be buildable by Go 1.4.
