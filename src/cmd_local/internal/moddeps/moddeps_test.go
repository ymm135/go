...
func TestAllDependencies(t *testing.T) {
	...
	if haveDiff := func() bool {
		...
		// would test the cmd_local/internal/moddeps package directly, and all.bash
	...
	// module version specified in GOROOT/src/cmd_local/go.mod.
	bundleDir := t.TempDir()
	r := runner{Dir: filepath.Join(runtime.GOROOT(), "src/cmd")}
	r.run(t, goBin, "build", "-mod=readonly", "-o", bundleDir, "golang.org/x/tools/cmd_local/bundle")
	...
	for _, m := range findGorootModules(t) {
		...
		t.Run(m.Path+"(thorough)", func(t *testing.T) {
			...
			if m.Path == "std" {
				r.run(t, goBinCopy, "generate", "syscall", "internal_local/syscall/...") // See issue 43440.
