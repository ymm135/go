...
const symName = "cmd_local/addr2line.TestAddr2Line"
...
func TestAddr2Line(t *testing.T) {
	...
	out, err := exec.Command(testenv.GoToolPath(t), "test", "-c", "-o", exepath, "cmd_local/addr2line").CombinedOutput()
	if err != nil {
		t.Fatalf("go test -c -o %v cmd_local/addr2line: %v\n%s", exepath, err, string(out))
	}
	os.Args[0] = exepath
	...
	out, err = exec.Command(testenv.GoToolPath(t), "build", "-o", exepath, "cmd_local/addr2line").CombinedOutput()
	if err != nil {
		t.Fatalf("go build -o %v cmd_local/addr2line: %v\n%s", exepath, err, string(out))
	}
