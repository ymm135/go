...
func genOp() {
	...
	fmt.Fprintln(w, "\"cmd_local/internal/obj\"")
	for _, a := range archs {
		if a.pkg != "" {
			fmt.Fprintf(w, "%q\n", a.pkg)
		}
	...
	for _, a := range archs {
		...
		for i, r := range a.regnames {
			pkg := a.pkg[len("cmd_local/internal/obj/"):]
			var objname string // name in cmd_local/internal/obj/$ARCH
			switch r {
			case "SB":
				// SB isn't a real register.  cmd_local/internal/obj expects 0 in this case.
				objname = "0"
			case "SP":
				objname = pkg + ".REGSP"
			case "g":
