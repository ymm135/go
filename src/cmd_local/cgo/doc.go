...
files:
	...
	x.cgo1.go       # for gc (cmd_local/compile)
	y.cgo1.go       # for gc
...
they might depend on (in the case of puts, stdio). cmd_local/link has been
...
presented to cmd_local/link as part of a larger program, contains:

	_go_.o        # gc-compiled object for _cgo_gotypes.go, _cgo_import.go, *.cgo1.go
	_all.o        # gcc-compiled object for _cgo_export.c, *.cgo2.c

The final program will be a dynamic executable, so that cmd_local/link can avoid
needing to process arbitrary .o files. It only needs to process the .o
files generated from C files that cgo writes, and those are much more
limited in the ELF or other features that they use.

In essence, the _cgo_import.o file includes the extra linking
directives that cmd_local/link is not sophisticated enough to derive from _all.o
on its own. Similarly, the _all.o uses dynamic references to real
system object code because cmd_local/link is not sophisticated enough to process
the real code.

The main benefits of this system are that cmd_local/link remains relatively simple
...
The text above describes "internal" linking, in which cmd_local/link parses and
links host object files (ELF, Mach-O, PE, and so on) into the final
executable itself. Keeping cmd_local/link simple means we cannot possibly
...
using internal linking, cmd_local/link can generate Go binaries by itself.

In order to allow linking arbitrary object files without requiring
dynamic libraries, cgo supports an "external" linking mode too. In
external linking mode, cmd_local/link does not process any host object files.
...
through to cmd_local/link. These are communicated by writing //go: directives in a
...
In internal mode, cmd_local/link itself processes all the host object files, in
...
In external mode, cmd_local/link does not process any host object files, in
...
that, cmd_local/link will discover that there is no definition for
_cgo_gcc_Cfunc_sin, referred to by the gc-compiled source file. This
is okay, because cmd_local/link also processes the cgo_import_static directive and
knows that _cgo_gcc_Cfunc_sin is expected to be supplied by a host
object file, so cmd_local/link does not treat the missing symbol as an error when
creating go.o. Indeed, the definition for _cgo_gcc_Cfunc_sin will be
provided to the host linker by foo2.cgo.o, which in turn will need the
symbol 'sin'. cmd_local/link also processes the cgo_ldflag directives, so that it
...
cmd_local/link Command Line Interface

The go command and any other Go-aware build systems invoke cmd_local/link
to link a collection of packages into a single binary. By default, cmd_local/link will
present the same interface it does today:

	cmd_local/link main.a

produces a file named a.out, even if cmd_local/link does so by invoking the host
linker in external linking mode.

By default, cmd_local/link will decide the linking mode as follows: if the only
packages using cgo are those on a list of known standard library
packages (net, os/user, runtime/cgo), cmd_local/link will use internal linking
mode. Otherwise, there are non-standard cgo packages involved, and cmd_local/link
will use external linking mode. The first rule means that a build of
the godoc binary, which uses net but no other cgo, can run without
needing gcc available. The second rule means that a build of a
cgo-wrapped library like sqlite3 can generate a standalone executable
instead of needing to refer to a dynamic library. The specific choice
can be overridden using a command line flag: cmd_local/link -linkmode=internal or
cmd_local/link -linkmode=external.

In an external link, cmd_local/link will create a temporary directory, write any
host object files found in package archives to that directory (renamed
to avoid conflicts), write the go.o file to that directory, and invoke
the host linker. The default value for the host linker is $CC, split
into fields, or else "gcc". The specific host linker command line can
be overridden using command line flags: cmd_local/link -extld=clang
-extldflags='-ggdb -O3'. If any package in a build includes a .cc or
other file compiled by the C++ compiler, the go tool will use the
-extld option to set the host linker to the C++ compiler.

These defaults mean that Go-aware build systems can ignore the linking
changes and keep running plain 'cmd_local/link' and get reasonable results, but
