...
import (
	...
	"cmd_local/go/internal/fsys"
	"cmd_local/go/internal/modinfo"
	"cmd_local/go/internal/modload"
	"cmd_local/go/internal/par"
	"cmd_local/go/internal/search"
...
func findInternal(path string) (index int, ok bool) {
	...
	case path == "internal", strings.HasPrefix(path, "internal_local/"):
		return 0, true
	}
	return 0, false
}
...
const (
	ToTool    TargetDir = iota // to GOROOT/pkg/tool (default for cmd_local/*)
	ToBin                      // to bin dir inside package root (default for non-cmd_local/*)
...
func InstallTargetDir(p *Package) TargetDir {
	if strings.HasPrefix(p.ImportPath, "code.google.com/p/go.tools/cmd_local/") {
		return StalePath
	}
	if p.Goroot && strings.HasPrefix(p.ImportPath, "cmd_local/") && p.Name == "main" {
		switch p.ImportPath {
		case "cmd_local/go", "cmd_local/gofmt":
			return ToBin
		}
		return ToTool
	}
...
func (p *Package) exeFromImportPath() string {
	...
	if cfg.ModulesEnabled {
		// If this is example.com/mycmd_local/v2, it's more useful to
...
// There is a copy of this function in cmd_local/compile/internal/gc/noder.go.
