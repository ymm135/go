...
OverlayLoop:
	...
	// This is read by readGccgoArchive in cmd_local/internal/buildid/buildid.go.
...
// defaultPkgConfig is defined in zdefaultcc.go, written by cmd_local/dist.
...
//
// (The build of cmd_local/go running under its own test is forbidden from installing
...
func (b *Builder) gccld(a *Action, p *load.Package, objdir, outfile string, flags []string, objs []string) error {
	...
	if len(out) > 0 {
		...
		// See also cmd_local/link/internal/ld's hostlink method.
		var save [][]byte
		var skipLines int
...
// defaultCC is defined in zdefaultcc.go, written by cmd_local/dist.
func (b *Builder) GccCmd(incdir, workdir string) []string {
	return b.compilerCmd(b.ccExe(), incdir, workdir)
}

// gxxCmd returns a g++ command line prefix
// defaultCXX is defined in zdefaultcc.go, written by cmd_local/dist.
