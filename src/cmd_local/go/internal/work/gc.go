...
func (gcToolchain) gc(b *Builder, a *Action, archive string, importcfg, embedcfg []byte, symabis string, asmhdr bool, gofiles []string) (ofile string, output []byte, err error) {
	...
	if p.Standard && (p.ImportPath == "internal_local/cpu" || p.ImportPath == "internal_local/bytealg") {
	...
	if p.Standard {
		switch p.ImportPath {
		case "bytes", "internal_local/poll", "net", "os":
			fallthrough
...
func (gcToolchain) ld(b *Builder, root *Action, out, importcfg, mainpkg string) error {
	...
	if root.Package.Goroot && strings.HasPrefix(root.Package.ImportPath, "cmd_local/") {
		...
		if !sys.MustLinkExternal(cfg.Goos, cfg.Goarch) {
			ldflags = append(ldflags, "-X=cmd_local/internal/objabi.buildID="+root.buildID)
		}
	}
