...
// used by the build artifact cache (see cmd_local/go/internal/cache), but
...
// build IDs and the overall compiler binary. See cmd_local/dist's cmdbootstrap
// for the actual convergence sequence.
//
// The “one-element cache” purpose is a bit more complex for installed
// binaries. For a binary, like cmd_local/gofmt, there are two steps: compile
// cmd_local/gofmt/*.go into main.a, and then link main.a into the gofmt binary.
...
// in the binary.
//
// Keep in sync with src/cmd_local/buildid/buildid.go
