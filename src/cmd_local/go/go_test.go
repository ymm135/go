...
import (
	...
	"cmd_local/go/internal/cache"
	"cmd_local/go/internal/cfg"
	"cmd_local/go/internal/robustio"
...
// testGOROOT is the GOROOT to use when running testgo, a cmd_local/go binary
...
func TestMain(m *testing.M) {
	// $GO_GCFLAGS a compiler debug flag known to cmd_local/dist, make.bash, etc.
	// It is not a standard go command flag; use os.Getenv, not cfg.Getenv.
	if os.Getenv("GO_GCFLAGS") != "" {
		fmt.Fprintf(os.Stderr, "testing: warning: no tests to run\n") // magic string for cmd_local/go
		fmt.Printf("cmd_local/go test is not compatible with $GO_GCFLAGS being set\n")
		fmt.Printf("SKIP\n")
		return
	}
	...
	if testenv.HasGoBuild() {
		...
		if hostGOOS != runtime.GOOS || hostGOARCH != runtime.GOARCH {
			fmt.Fprintf(os.Stderr, "testing: warning: no tests to run\n") // magic string for cmd_local/go
			fmt.Printf("cmd_local/go test is not compatible with GOOS/GOARCH != GOHOSTOS/GOHOSTARCH (%s/%s != %s/%s)\n", runtime.GOOS, runtime.GOARCH, hostGOOS, hostGOARCH)
			fmt.Printf("SKIP\n")
			return
		}
...
// cmd_local/go: custom import path checking should not apply to Go packages without import comment.
...
func TestGoListStdDoesNotIncludeCommands(t *testing.T) {
	...
	tg.parallel()
	tg.run("list", "std")
	tg.grepStdoutNot("cmd_local/", "go list std shows commands")
}
...
func TestGoListCmdOnlyShowsCommands(t *testing.T) {
	...
	for _, line := range strings.Split(out, "\n") {
		if !strings.Contains(line, "cmd_local/") {
...
func TestGoListDeps(t *testing.T) {
	...
	if runtime.Compiler != "gccgo" {
		...
		want := "internal_local/cpu\nunsafe\nmath/bits\nmath\n"
		out := tg.stdout.String()
		if !strings.Contains(out, "internal_local/cpu") {
...
func TestGoListTest(t *testing.T) {
	...
	tg.run("list", "-test", "cmd_local/dist", "cmd_local/doc")
	tg.grepStdout(`^cmd_local/dist$`, "missing cmd_local/dist")
	tg.grepStdout(`^cmd_local/doc$`, "missing cmd_local/doc")
	tg.grepStdout(`^cmd_local/doc\.test$`, "missing cmd_local/doc test")
	tg.grepStdoutNot(`^cmd_local/dist\.test$`, "unexpected cmd_local/dist test")
...
// Issue 18225.
// This is really a cmd_local/asm issue but this is a convenient place to test it.
func TestConcurrentAsm(t *testing.T) {
	skipIfGccgo(t, "gccgo does not use cmd_local/asm")
	tg := testgo(t)
	defer tg.cleanup()
	tg.parallel()
...
// Issue 19198.
// This is really a cmd_local/link issue but this is a convenient place to test it.
...
func TestNeedVersion(t *testing.T) {
	skipIfGccgo(t, "gccgo does not use cmd_local/compile")
	tg := testgo(t)
	defer tg.cleanup()
	tg.parallel()
...
func testBuildmodePIE(t *testing.T, useCgo, setBuildmodeToPIE bool) {
	...
	case "windows":
		...
		if useCgo {
			...
			// TODO: deduplicate this struct from cmd_local/link/internal/ld/pe.go
...
func TestLinkerTmpDirIsDeleted(t *testing.T) {
	skipIfGccgo(t, "gccgo does not use cmd_local/link")
	if !canCgo {
		t.Skip("skipping because cgo not enabled")
	}
	tooSlow(t)
