...
import (
	"bytes"
	cmddwarf "cmd_local/internal/dwarf"
	"cmd_local/internal/objfile"
	"debug/dwarf"
...
func testDWARF(t *testing.T, buildmode string, expectDWARF bool, env ...string) {
	...
	out, err := exec.Command(testenv.GoToolPath(t), "list", "-f", "{{.Stale}}", "cmd_local/link").CombinedOutput()
	if err != nil {
		t.Fatalf("go list: %v\n%s", err, out)
	}
	if string(out) != "false\n" {
		if strings.HasPrefix(testenv.Builder(), "darwin-") {
			t.Skipf("cmd_local/link is spuriously stale on Darwin builders - see #33598")
		}
		t.Fatalf("cmd_local/link is stale - run go install cmd_local/link")
	}
