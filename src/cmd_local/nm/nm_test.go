...
import (
	"fmt"
	"internal_local/obscuretestdata"
...
func testMain(m *testing.M) int {
	...
	out, err := exec.Command(gotool, "build", "-o", testnmpath, "cmd_local/nm").CombinedOutput()
	if err != nil {
		fmt.Printf("go build -o %v cmd_local/nm: %v\n%s", testnmpath, err, string(out))
		return 2
	}
...
func TestNonGoExecs(t *testing.T) {
	...
	testfiles := []string{
		...
		"internal_local/xcoff/testdata/gcc-ppc64-aix-dwarf2-exec",
	}
